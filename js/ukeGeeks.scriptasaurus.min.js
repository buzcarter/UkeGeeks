...
return c[1].replace(regEx.dbleSpace,' ');};var _getTuning=function(text){var c=text.match(regEx.tuning);if(!c){return null;}
...
var m=(f[1].length==4)?f[1].match(regEx.any):f[1].match(regEx.numOrX);var j=0;for(var i in m){var isX=m[i]=='x'||m[i]=='X';frets[j]=isX?0:parseInt(m[i]);muted[j]=isX;j++;}};var _getFingers=function(text){var f=text.match(regEx.fingers);if(!f){return[];}
...
return chords;},_encloseChords:function(text,chords){for(var i in chords){do{}
while(text.length!=(text=text.replace('['+chords[i]+']','<code data-chordName="'+chords[i]+'"><strong>[<em>'+chords[i]+'</em>]</strong></code>')).length);}
...
return song;},regEx:{blocks:/\s*{\s*(start_of_tab|sot|start_of_chorus|soc|end_of_tab|eot|end_of_chorus|eoc)\s*}\s*/im,tabBlock:/\s*{\s*(start_of_tab|sot)\s*}\s*/im,chorusBlock:/\s*{\s*(start_of_chorus|soc)\s*}\s*/im},classNames:{Comment:'ugsComment',Tabs:'ugsTabs',Chorus:'ugsChorus',PreChords:'ugsChords',PrePlain:'ugsPlain',NoLyrics:'ugsNoLyrics',ColumnWrap:'ugsWrap',ColumnCount:'ugsColumnCount',Column:'ugsColumn'},blockTypeEnum:{TextBlock:1,ChorusBlock:2,TabBlock:3,Comment:101,Title:102,Subtitle:103,Album:104,ChordDefinition:105,UkeGeeksMeta:106,ColumnBreak:107,ChordText:201,PlainText:202,ChordOnlyText:203,Undefined:666},_getBlockType:function(line){if(this.regEx.chorusBlock.test(line)){return this.blockTypeEnum.ChorusBlock;}
...
return html;},_echo:function(song){for(var i in song){console.log('>> '+i+'. '+song[i].type+' node, '+song[i].lines.length+' lines');for(var j in song[i].lines){console.log(song[i].lines[j]);}}},_domParse:function(text){var lines=text.split('\n');var song=[];var tmpBlk=null;var isMarker;for(var i in lines){isMarker=this.regEx.blocks.test(lines[i]);if(isMarker||tmpBlk==null){if(tmpBlk!=null){song.push(tmpBlk);}
tmpBlk={type:this._getBlockType(lines[i]),lines:[]};if(!isMarker){tmpBlk.lines.push(lines[i]);}}
else{var s=this.trim(lines[i]);if(s.length>0){tmpBlk.lines.push(s);}}}
if(tmpBlk.lines.length>0){song.push(tmpBlk);}
return song;},_parseInstr:function(song){var regEx={instr:/\{[^}]+?:.*?\}/im,cmdArgs:/\{.+?:(.*)\}/gi,cmdVerb:/\{(.+?):.*\}/gi};for(var i in song){for(var j in song[i].lines){if(regEx.instr.test(song[i].lines[j])){var args=song[i].lines[j].replace(regEx.cmdArgs,'$1');var verb=song[i].lines[j].replace(regEx.cmdVerb,'$1').toLowerCase();var tmpBlk={type:'',lines:[]};switch(verb){case'title':case't':tmpBlk.type=this.blockTypeEnum.Title;break;case'subtitle':case'st':tmpBlk.type=this.blockTypeEnum.Subtitle;break;case'album':tmpBlk.type=this.blockTypeEnum.Album;break;case'comment':case'c':tmpBlk.type=this.blockTypeEnum.Comment;break;case'define':tmpBlk.type=this.blockTypeEnum.ChordDefinition;break;case'ukegeeks-meta':tmpBlk.type=this.blockTypeEnum.UkeGeeksMeta;break;default:tmpBlk.type='Undefined-'+verb;break;}
tmpBlk.lines[0]=this.trim(args);song[i].lines[j]=tmpBlk;}}}
return song;},_parseSimpleInstr:function(song){var regEx={columnBreak:/\s*{\s*(column_break|colb)\s*}\s*/im};for(var i in song){for(var j in song[i].lines){if(regEx.columnBreak.test(song[i].lines[j])){this.columnCount++;song[i].lines[j]={type:this.blockTypeEnum.ColumnBreak,lines:[]};}}}
return song;},_markChordLines:function(song){var regEx={chord:/\[(.+?)]/i,allChords:/\[(.+?)]/img,trim:/^\s+|\s+$/g};var hasChrd;var isChrdOnly;var line;for(var i in song){if(song[i].type==this.blockTypeEnum.TextBlock||song[i].type==this.blockTypeEnum.ChorusBlock){for(var j in song[i].lines){line=song[i].lines[j];if(typeof(line)=='string'){hasChrd=regEx.chord.test(line);this.hasChords=this.hasChords||hasChrd;isChrdOnly=hasChrd&&(line.replace(regEx.allChords,'').replace(regEx.trim,'').length<1);song[i].lines[j]={type:(isChrdOnly?this.blockTypeEnum.ChordOnlyText:(hasChrd?this.blockTypeEnum.ChordText:this.blockTypeEnum.PlainText)),lines:[line]};}}}}
...
return rtn;},trim:function(str){return str.replace(/^\s+|\s+$/g,'');},_stripHtml:function(text){var regEx={pre:/<\/?pre>/img,htmlComment:/<!--(.|\n)*?-->/gm};return text.replace(regEx.pre,'').replace(regEx.htmlComment,'');}};
;ukeGeeks.chordPainter=function(){};ukeGeeks.chordPainter.prototype={brush:null,errors:[],init:function(){this.brush=new ukeGeeks.chordBrush;this.brush.init();},show:function(id,chords){var chordBox=document.getElementById(id);if(!chordBox)return;chordBox.innerHTML='';this.errors=[];for(var i=0;i<chords.length;i++){var c=ukeGeeks.definitions.get(chords[i]);if(!c){this.errors.push(chords[i]);continue;}
this.brush.plot(chordBox,c,ukeGeeks.settings.fretBox);}},showInline:function(chords){var e=document.getElementById(ukeGeeks.settings.ids.songText).getElementsByTagName('code');if(e.length<1)return;for(var i=0;i<chords.length;i++){var c=ukeGeeks.definitions.get(chords[i]);if(!c){continue;}
for(var j=0;j<e.length;j++){if(e[j].getAttribute('data-chordName')==c.name){this.brush.plot(e[j],c,ukeGeeks.settings.inlineFretBox);}}}},getErrors:function(){return this.errors;}}
;ukeGeeks.tabs=function(){};ukeGeeks.tabs.prototype={init:function(){},trim:function(str){return str.replace(/^\s+|\s+$/g,'');},replace:function(h){var tabBlocks=h.getElementsByTagName('pre');for(var i in tabBlocks){if(tabBlocks[i].className=='ugsTabs'){var s=tabBlocks[i].innerHTML;tabBlocks[i].innerHTML="";this.loadBlocks(s,tabBlocks[i]);}}},loadBlocks:function(text,outElement){var lns=text.split('\n');var tab=[];for(var i in lns){var s=this.trim(lns[i]);if(s.length>0){tab.push(s);}
if(tab.length==4){this.redraw(tab,outElement);tab=[];}}},redraw:function(inTabs,outElement){inTabs=(typeof(inTabs)=='string')?(inTabs.split('\n')):inTabs;if(inTabs.length<4)return;var tabInfo=this._readTabs(inTabs);var labelOffset=(tabInfo.hasLabels)?ukeGeeks.settings.tabs.labelWidth:0;var tabs=tabInfo.tabs;var length=(ukeGeeks.settings.tabs.noteSpacing*tabs[0].length+labelOffset)+(ukeGeeks.settings.tabs.dotRadius);var height=(3*ukeGeeks.settings.tabs.lineSpacing)+(2*ukeGeeks.settings.tabs.dotRadius)+ukeGeeks.settings.tabs.bottomPadding;outElement=(typeof(outElement)=='string')?document.getElementById(outElement):outElement;var ctx=ukeGeeks.canvasTools.addCanvas(outElement,length,height);var pos={x:ukeGeeks.settings.tabs.dotRadius+labelOffset,y:1+ukeGeeks.settings.tabs.dotRadius};this._drawStaff(ctx,pos,length,ukeGeeks.settings.tabs);this._drawNotes(ctx,pos,tabs,ukeGeeks.settings.tabs);if(tabInfo.hasLabels){this._drawLabels(ctx,pos,ukeGeeks.settings.tabs);}},_readTabs:function(ukeStrings){var hasLabels=ukeStrings[3][0]=='G';if(hasLabels){this._rdTbStripLabels(ukeStrings);}
var frets=this._rdTbGetFrets(ukeStrings);var symbols=this._rdTbGetSymbols(ukeStrings);var minLength=this._rdTbGetMinLength(ukeStrings);var guide=this._rdTbGetGuide(symbols,minLength);return{tabs:this._rdTbGetPacked(frets,symbols,guide,minLength),hasLabels:hasLabels};},_rdTbStripLabels:function(ukeStrings){for(var i=0;i<4;i++){ukeStrings[i]=ukeStrings[i].substr(1);}},_rdTbGetFrets:function(ukeStrings){var reInts=/([0-9]+)/g;var frets=[];for(var i=0;i<4;i++){frets[i]=ukeStrings[i].match(reInts);}
...
ctx.strokeStyle=settings.lineColor;ctx.lineWidth=settings.lineWidth;ctx.stroke();ctx.closePath();},_drawNotes:function(ctx,pos,tabs,settings){var c;var center={x:0,y:pos.y};for(var i in tabs){if(i>3)return;center.x=pos.x;for(var j in tabs[i]){c=tabs[i][j];if(c=='|'){this._drawMeasure(ctx,{x:center.x,y:pos.y},settings);}
...
center.y+=settings.lineSpacing;}},_drawMeasure:function(ctx,pos,settings){var offset=settings.lineWidth/2;ctx.beginPath();ctx.moveTo(pos.x+offset,pos.y);ctx.lineTo(pos.x+offset,pos.y+3*settings.lineSpacing);ctx.strokeStyle=settings.lineColor;ctx.lineWidth=settings.lineWidth;ctx.stroke();ctx.closePath();},_drawLabels:function(ctx,pos,settings){var labels=ukeGeeks.settings.tuning.reverse();for(var i=0;i<4;i++){ukeGeeks.canvasTools.drawText(ctx,{x:1,y:(pos.y+(i+0.3)*settings.lineSpacing)},labels[i],settings.labelFont,settings.lineColor,'left');}}}
;ukeGeeks.scriptasaurus=new function(){this.init=function(isIeFamily){ukeGeeks.settings.environment.isIe=isIeFamily;};this.run=function(offset){var offset=(arguments.length>0)?arguments[0]:ukeGeeks.definitions.instrument.sopranoUke;var h=document.getElementById(ukeGeeks.settings.ids.songText);if(!h)return;ukeGeeks.definitions.useInstrument(offset);var cpm=new ukeGeeks.cpmParser;cpm.init();var song=cpm.parse(h.innerHTML);ukeGeeks.definitions.replace(song.defs);var chrdPrsr=new ukeGeeks.chordParser;chrdPrsr.init();h.innerHTML=chrdPrsr.parse(song.body);var chordsInUse=chrdPrsr.getChords();var painter=new ukeGeeks.chordPainter;painter.init();painter.show(ukeGeeks.settings.ids.canvas,chordsInUse);if(ukeGeeks.settings.inlineDiagrams){var b=document.getElementsByTagName('body')[0];if(b.className.search('ugsInlineDiagrams')<0){b.className+=" ugsInlineDiagrams ";}
painter.showInline(chordsInUse);}
var tabs=new ukeGeeks.tabs;tabs.init();tabs.replace(h);showErrors(painter.getErrors());return song;};var showErrors=function(errs){if(errs.length>0){var s='';for(var i=0;i<errs.length;i++){s+=errs[i]+(((i+1)==errs.length)?'':', ');}

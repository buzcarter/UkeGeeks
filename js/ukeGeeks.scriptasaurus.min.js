...
;ukeGeeks.chordPainter=function(){};ukeGeeks.chordPainter.prototype={brush:null,errors:[],handles:null,init:function(handles){this.brush=new ukeGeeks.chordBrush;this.brush.init();this.handles=handles;},show:function(chords){this.handles.diagrams.innerHTML='';this.errors=[];for(var i=0;i<chords.length;i++){var c=ukeGeeks.definitions.get(chords[i]);if(!c){this.errors.push(chords[i]);continue;}
this.brush.plot(this.handles.diagrams,c,ukeGeeks.settings.fretBox);}},showInline:function(chords){var e=this.handles.text.getElementsByTagName('code');if(e.length<1)return;for(var i=0;i<chords.length;i++){var c=ukeGeeks.definitions.get(chords[i]);if(!c){continue;}
...
;ukeGeeks.scriptasaurus=new function(){this.init=function(isIeFamily){ukeGeeks.settings.environment.isIe=isIeFamily;ukeGeeks.definitions.useInstrument(ukeGeeks.definitions.instrument.sopranoUke);};this.run=function(){var handles=_getHandlesFromId();if(!handles||!handles.diagrams||!handles.text||!handles.wrap){return null;}
...
return songs;};this.setTuningOffset=function(offset){ukeGeeks.definitions.useInstrument(offset);};var _errList=[];var _runSong=function(handles){var cpm=new ukeGeeks.cpmParser;cpm.init();var song=cpm.parse(handles.text.innerHTML);ukeGeeks.definitions.replace(song.defs);var chrdPrsr=new ukeGeeks.chordParser;chrdPrsr.init();handles.text.innerHTML=chrdPrsr.parse(song.body);var chordsInUse=chrdPrsr.getChords();var painter=new ukeGeeks.chordPainter;painter.init(handles);painter.show(chordsInUse);if(ukeGeeks.settings.inlineDiagrams){ukeGeeks.toolsLite.addClass(handles.wrap,'ugsInlineDiagrams');painter.showInline(chordsInUse);}
...
console.log(typeof(errs[0]));var s='';for(var i=0;i<errs.length;i++){s+=(s.length>0)?', ':'';s+=errs[i];}

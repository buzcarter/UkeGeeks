...
return song;},regEx:{blocks:/\s*{\s*(start_of_tab|sot|start_of_chorus|soc|end_of_tab|eot|end_of_chorus|eoc)\s*}\s*/im,tabBlock:/\s*{\s*(start_of_tab|sot)\s*}\s*/im,chorusBlock:/\s*{\s*(start_of_chorus|soc)\s*}\s*/im},classNames:{Comment:'ugsComment',Tabs:'ugsTabs',Chorus:'ugsChorus',PreChords:'ugsChords',PrePlain:'ugsPlain',NoLyrics:'ugsNoLyrics',ColumnWrap:'ugsWrap',ColumnCount:'ugsColumnCount',Column:'ugsColumn'},blockTypeEnum:{TextBlock:1,ChorusBlock:2,TabBlock:3,Comment:101,Title:102,Subtitle:103,Album:104,ChordDefinition:105,UkeGeeksMeta:106,ColumnBreak:107,Artist:108,ChordText:201,PlainText:202,ChordOnlyText:203,Undefined:666},_getBlockType:function(line){if(this.regEx.chorusBlock.test(line)){return this.blockTypeEnum.ChorusBlock;}
...
;ukeGeeks.chordPainter=function(){};ukeGeeks.chordPainter.prototype={brush:null,errors:[],nodes:null,init:function(domNodes){this.brush=new ukeGeeks.chordBrush;this.brush.init();this.nodes=domNodes;},show:function(chords){this.nodes.diagrams.innerHTML='';this.errors=[];for(var i=0;i<chords.length;i++){var c=ukeGeeks.definitions.get(chords[i]);if(!c){this.errors.push(chords[i]);continue;}
this.brush.plot(this.nodes.diagrams,c,ukeGeeks.settings.fretBox);}},showInline:function(chords){var e=this.nodes.text.getElementsByTagName('code');if(e.length<1)return;for(var i=0;i<chords.length;i++){var c=ukeGeeks.definitions.get(chords[i]);if(!c){continue;}
...
;ukeGeeks.scriptasaurus=new function(){this.init=function(isIeFamily){ukeGeeks.settings.environment.isIe=isIeFamily;ukeGeeks.definitions.useInstrument(ukeGeeks.definitions.instrument.sopranoUke);};this.run=function(){var node=_makeNodeById();if(!node.diagrams||!node.text||!node.wrap){return null;}
return _runSong(node);};this.runByClasses=function(){var songs=[];var songWraps=ukeGeeks.toolsLite.getElementsByClass(ukeGeeks.settings.wrapClasses.wrap);for(var i=0;i<songWraps.length;i++){var node=_makeNodeByClass(songWraps[i]);if(node==null){continue;}
songs.push(_runSong(node));}
return songs;};this.setTuningOffset=function(offset){ukeGeeks.definitions.useInstrument(offset);};var _runSong=function(nodes){var cpm=new ukeGeeks.cpmParser;cpm.init();var song=cpm.parse(nodes.text.innerHTML);ukeGeeks.definitions.replace(song.defs);var chrdPrsr=new ukeGeeks.chordParser;chrdPrsr.init();nodes.text.innerHTML=chrdPrsr.parse(song.body);var chordsInUse=chrdPrsr.getChords();var painter=new ukeGeeks.chordPainter;painter.init(nodes);painter.show(chordsInUse);if(ukeGeeks.settings.inlineDiagrams){ukeGeeks.toolsLite.addClass(nodes.wrap,'ugsInlineDiagrams');painter.showInline(chordsInUse);}
var tabs=new ukeGeeks.tabs;tabs.init();tabs.replace(nodes.text);var container=nodes.wrap;if(container){if(!song.hasChords){ukeGeeks.toolsLite.addClass(container,'ugsNoChords');}
else{ukeGeeks.toolsLite.removeClass(container,'ugsNoChords');}}
return song;};var _makeNodeByClass=function(wrap){var diagrams=ukeGeeks.toolsLite.getElementsByClass(ukeGeeks.settings.wrapClasses.diagrams,wrap);var text=ukeGeeks.toolsLite.getElementsByClass(ukeGeeks.settings.wrapClasses.text,wrap);if((diagrams==undefined)||(diagrams.length<1)||(text==undefined)||(text.length<1)){return null;}
var node={wrap:wrap,diagrams:diagrams[0],text:text[0]};return node;};var _makeNodeById=function(){var node={wrap:document.getElementById(ukeGeeks.settings.ids.container),diagrams:document.getElementById(ukeGeeks.settings.ids.canvas),text:document.getElementById(ukeGeeks.settings.ids.songText)};return node;}}
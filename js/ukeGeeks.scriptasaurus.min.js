...
;ukeGeeks.definitions=(function(){var _public={};var _userChords=[];var _chords=[];var _instruments=[];var _offset=0;var _map=[];_public.instrument={sopranoUke:0,baritoneUke:5};_public.addInstrument=function(text){_instruments.push(text);};_public.useInstrument=function(offset){offset=(arguments.length>0)?arguments[0]:_public.instrument.sopranoUke;_offset=parseInt(offset,10);if(_offset>0){_map=ukeGeeks.transpose.retune(_offset);}
_public.setChords(ukeGeeks.chordImport.runBlock(_instruments[0]).chords);};_public.get=function(chordName){var i;for(i=0;i<_userChords.length;i++){if(chordName==_userChords[i].name){return _userChords[i];}}
if(_offset<1){return _get(chordName);}
var name=_getAlias(chordName);for(i in _map){if(name==_map[i].original){var c=_get(_map[i].transposed);if(c){var chrd=new ukeGeeks.data.expandedChord(chordName);chrd.dots=c.dots;chrd.muted=c.muted;return chrd;}}}
return null;};var _aliases={'A#':'Bb','Db':'C#','D#':'Eb','Gb':'F#','Ab':'G#'};var _getAlias=function(chordName){var n=chordName.substr(0,2);return!_aliases[n]?chordName:_aliases[n]+chordName.substr(2);};var _get=function(chordName){var name=_getAlias(chordName);for(var i=0;i<_chords.length;i++){if(name==_chords[i].name){var chrd=new ukeGeeks.data.expandedChord(chordName);chrd.dots=_chords[i].dots;chrd.muted=_chords[i].muted;return chrd;}}
...
var size=parseInt(bits[1])*mulitplier;return font.replace(sizeRe,size+bits[2]);};_public.scale=function(mulitplier){if(mulitplier==1.0){return;}
...
else{_public.removeClass(element,className);}};var getRegEx=function(className){return new RegExp('(\\s|^)'+className+'(\\s|$)');};_public.trim=function(str){return str.replace(regEx.trim,'');};_public.pack=function(value){return value.replace(regEx.dbleSpace,' ').replace(regEx.trim,'');};_public.getElementsByClass=function(searchClass,node,tag){if(!node){node=document;}
...
return newChords;};return _public;}());;ukeGeeks.definitions.addInstrument("\
...
");;ukeGeeks.canvasTools=(function(){var _public={};_public.drawDot=function(ctx,centerPos,radius,color){ctx.beginPath();ctx.arc(centerPos.x,centerPos.y,radius,0,Math.PI*2,true);ctx.closePath();ctx.fillStyle=color;ctx.fill();};_public.drawText=function(ctx,pos,text,font,color,align){if(!ctx.fillText)return;ctx.font=font;ctx.textAlign=(align||'center');ctx.fillStyle=color;ctx.fillText(text,pos.x,pos.y);};_public.addCanvas=function(element,width,height){var c=document.createElement('canvas');if(!c){return null;}
...
return ctx;};return _public;}());;ukeGeeks.chordBrush=function(){var publics={};publics.init=function(){};publics.plot=function(chordBox,chord,fretBox,fontSettings,colorSettings){var ctx=ukeGeeks.canvasTools.addCanvas(chordBox,fretBox.width,fretBox.height);if(ctx==null){return;}
...
return{first:(min<300)?min:0,last:(max>0)?max:0};};return publics;};;ukeGeeks.chordParser=function(){};ukeGeeks.chordParser.prototype={chords:[],init:function(){},parse:function(text){this.chords=this._findChords(text);text=this._encloseChords(text,this.chords);text=this._packChords(text);return text;},getChords:function(){return this.chords;},_findChords:function(text){var re=/\[(.+?)]/img;var m=text.match(re);if(!m)return[];var chords=[];var found;for(var i=0;i<m.length;i++){found=false;for(var j=0;j<chords.length;j++){if(chords[j]==m[i]){found=true;break;}}
...
return rtn;},_stripHtml:function(text){var regEx={pre:/<\/?pre>/img,htmlComment:/<!--(.|\n)*?-->/gm};return text.replace(regEx.pre,'').replace(regEx.htmlComment,'');}};;ukeGeeks.chordPainter=function(){var publics={};var brush=null;var errors=[];var handles=null;publics.init=function(htmlHandles){brush=new ukeGeeks.chordBrush();brush.init();handles=htmlHandles;};var ignoreMatchList=[];var _tacet=/^(n.?\/?c.?|tacet)$/i;var ignoreChord=function(chord){for(var i=0;i<ukeGeeks.settings.commonChords.length;i++){if(chord==ukeGeeks.settings.commonChords[i]){return true;}}
return false;};publics.show=function(chords){handles.diagrams.innerHTML='';errors=[];ignoreMatchList=[];for(var i=0;i<chords.length;i++){if(_tacet.test(chords[i])){continue;}
...
var boxA=getBox(strongA);var boxB=getBox(strongB);if(checkOverlap(boxA,boxB)){var width=boxA.right-boxB.left+1;codeA.style.paddingRight=(width<1?1:width)+'px';}};_public.Fix=function(ele){var elements=ele.getElementsByTagName('code');for(var i=0;i<elements.length;i++){elements[i].style.paddingRight='0px';}
for(var i=0;i<(elements.length-1);i++){checkChords(elements[i],elements[i+1]);}};return _public;})();;ukeGeeks.scriptasaurus=(function(){var _public={};_public.init=function(isIeFamily){ukeGeeks.settings.environment.isIe=isIeFamily;ukeGeeks.definitions.useInstrument(ukeGeeks.definitions.instrument.sopranoUke);if(ukeGeeks.settings.defaultInstrument!=ukeGeeks.definitions.instrument.sopranoUke){ukeGeeks.definitions.useInstrument(ukeGeeks.settings.defaultInstrument);}};_public.run=function(){var handles=_getHandlesFromId();if(!handles||!handles.diagrams||!handles.text||!handles.wrap){return null;}

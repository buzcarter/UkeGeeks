...
;ukeGeeks.scriptasaurus=new function(){this.init=function(isIeFamily){ukeGeeks.settings.environment.isIe=isIeFamily;};this.run=function(offset){var offset=(arguments.length>0)?arguments[0]:ukeGeeks.definitions.instrument.sopranoUke;var h=document.getElementById(ukeGeeks.settings.ids.songText);if(!h)return null;ukeGeeks.definitions.useInstrument(offset);var cpm=new ukeGeeks.cpmParser;cpm.init();var song=cpm.parse(h.innerHTML);ukeGeeks.definitions.replace(song.defs);var chrdPrsr=new ukeGeeks.chordParser;chrdPrsr.init();h.innerHTML=chrdPrsr.parse(song.body);var chordsInUse=chrdPrsr.getChords();var painter=new ukeGeeks.chordPainter;painter.init();painter.show(ukeGeeks.settings.ids.canvas,chordsInUse);if(ukeGeeks.settings.inlineDiagrams){var b=document.getElementsByTagName('body')[0];ukeGeeks.toolsLite.addClass(b,'ugsInlineDiagrams');painter.showInline(chordsInUse);}
var tabs=new ukeGeeks.tabs;tabs.init();tabs.replace(h);showErrors(painter.getErrors());var container=document.getElementById(ukeGeeks.settings.ids.container);if(container){if(!song.hasChords){ukeGeeks.toolsLite.addClass(container,'ugsNoChords');}
else{ukeGeeks.toolsLite.removeClass(container,'ugsNoChords');}}
return song;};var showErrors=function(errs){if(errs.length>0){var s='';for(var i=0;i<errs.length;i++){s+=errs[i]+(((i+1)==errs.length)?'':', ');}

...
;ukeGeeks.definitions=(function(){var _public={};var _userChords=[];var _chords=[];var _instruments=[];var _offset=0;var _map=[];_public.instrument={sopranoUke:0,baritoneUke:5};_public.addInstrument=function(text){_instruments.push(text);};_public.useInstrument=function(offset){offset=(arguments.length>0)?offset:_public.instrument.sopranoUke;_offset=parseInt(offset,10);if(_offset>0){_map=ukeGeeks.transpose.retune(_offset);}
...
var size=parseInt(bits[1],10)*mulitplier;return font.replace(sizeRe,size+bits[2]);};_public.scale=function(mulitplier){if(mulitplier==1.0){return;}
...
return c;};var _log=function(msg){_errs.push(msg);};var _errs=[];var _echoLog=function(){for(var i in _errs){console.log(i+'. '+_errs[i]);}};_public.runLine=function(line){var c=_lineToParts(line);return!c?null:_getExpandedChord(c.define,c.adds);};_public.runBlock=function(text){var nL=text.split('\n');if(nL.length<2){nL=text.split('{');}
var parts=_textToParts(nL);var n=_getInstrument(text);var t=_getTuning(text);return new ukeGeeks.data.instrument(_getKey(n,t),n,t,_partsToChords(parts));};return _public;}());;ukeGeeks.transpose=(function(){var _public={};var re=/^([A-G][#b]?)(.*)/;var tones={'A':0,'A#':1,'Bb':1,'B':2,'C':3,'C#':4,'Db':4,'D':5,'D#':6,'Eb':6,'E':7,'F':8,'F#':9,'Gb':9,'G':10,'G#':11,'Ab':11};_public.shift=function(name,steps){var t=getTone(name);if(t===null){return null;}
...
return{first:(min<300)?min:0,last:(max>0)?max:0};};return _public;};;ukeGeeks.chordParser=function(){};ukeGeeks.chordParser.prototype={chords:[],init:function(){},parse:function(text){this.chords=this._findChords(text);text=this._encloseChords(text,this.chords);text=this._packChords(text);return text;},getChords:function(){return this.chords;},_findChords:function(text){var i,j;var re=/\[(.+?)]/img;var m=text.match(re);if(!m)return[];var chords=[];var found;for(i=0;i<m.length;i++){found=false;for(j=0;j<chords.length;j++){if(chords[j]==m[i]){found=true;break;}}
...
return text;},_packChords:function(text){var re;if(ukeGeeks.settings.inlineDiagrams){re=/(<\/strong><\/code>)[ \t]*(<code data-chordName="[^"]*"><strong>)/ig;return text.replace(re,'$1<span class="ugsInlineSpacer">&nbsp;</span>$2');}
re=/<\/strong><\/code>[ \t]*<code data-chordName="[^"]*"><strong>/ig;return text.replace(re,' ');}};;ukeGeeks.cpmParser=function(){};ukeGeeks.cpmParser.prototype={runaway:30,columnCount:1,hasChords:false,init:function(){},parse:function(text){var song=new ukeGeeks.data.song();text=this._stripHtml(text);var songDom=this._domParse(text);songDom=this._parseInstr(songDom);songDom=this._parseSimpleInstr(songDom);songDom=this._markChordLines(songDom);song.body=this._export(songDom);if(this.columnCount>1){song.body='<div class="'+this.classNames.ColumnWrap+' '+this.classNames.ColumnCount+this.columnCount+'">'+'<div class="'+this.classNames.Column+'">'+song.body+'</div>'+'</div>';}
...
else if((song[i].type==this.blockTypeEnum.ChordText)||(song[i].type==this.blockTypeEnum.PlainText)||(song[i].type==this.blockTypeEnum.ChordOnlyText)){if(song[i].lines[0].length<1){continue;}
var myClass=(song[i].type==this.blockTypeEnum.PlainText)?this.classNames.PrePlain:this.classNames.PreChords;if(song[i].type==this.blockTypeEnum.ChordOnlyText){myClass+=' '+this.classNames.NoLyrics;}
...
return rtn;},_stripHtml:function(text){var regEx={pre:/<\/?pre>/img,htmlComment:/<!--(.|\n)*?-->/gm};return text.replace(regEx.pre,'').replace(regEx.htmlComment,'');}};;ukeGeeks.chordPainter=function(){var _public={};var brush=null;var errors=[];var handles=null;_public.init=function(htmlHandles){brush=new ukeGeeks.chordBrush();brush.init();handles=htmlHandles;};var ignoreMatchList=[];var _tacet=/^(n.?\/?c.?|tacet)$/i;var ignoreChord=function(chord){for(var i=0;i<ukeGeeks.settings.commonChords.length;i++){if(chord==ukeGeeks.settings.commonChords[i]){return true;}}
return false;};_public.show=function(chords){handles.diagrams.innerHTML='';errors=[];ignoreMatchList=[];for(var i=0;i<chords.length;i++){if(_tacet.test(chords[i])){continue;}
if(ukeGeeks.settings.opts.ignoreCommonChords&&ignoreChord(chords[i])){ignoreMatchList.push(chords[i]);continue;}
var c=ukeGeeks.definitions.get(chords[i]);if(!c){errors.push(chords[i]);continue;}
brush.plot(handles.diagrams,c,ukeGeeks.settings.fretBox);}};_public.showInline=function(chords){var e=handles.text.getElementsByTagName('code');if(e.length<1)return;for(var i=0;i<chords.length;i++){var c=ukeGeeks.definitions.get(chords[i]);if(!c){continue;}
for(var j=0;j<e.length;j++){if(e[j].getAttribute('data-chordName')==c.name){brush.plot(e[j],c,ukeGeeks.settings.inlineFretBox,ukeGeeks.settings.inlineFretBox.fonts);}}}};_public.getErrors=function(){return errors;};_public.getIgnoredChords=function(){return ignoreMatchList;};return _public;};;ukeGeeks.tabs=function(){};ukeGeeks.tabs.prototype={_numStrings:4,_lastStringName:'G',init:function(){},replace:function(h){var tabBlocks=h.getElementsByTagName('pre');for(var i in tabBlocks){if(tabBlocks[i].className=='ugsTabs'){var s=tabBlocks[i].innerHTML;tabBlocks[i].innerHTML='';this.loadBlocks(s,tabBlocks[i]);}}},loadBlocks:function(text,outElement){var lines=text.split('\n');var tab=[];for(var i in lines){var s=ukeGeeks.toolsLite.trim(lines[i]);if(s.length>0){tab.push(s);}
if(tab.length==this._numStrings){this.redraw(tab,outElement);tab=[];}}},redraw:function(inTabs,outElement){inTabs=(typeof(inTabs)=='string')?(inTabs.split('\n')):inTabs;if(inTabs.length<this._numStrings){return;}
var tabInfo=this._readTabs(inTabs);var labelOffset=(tabInfo.hasLabels)?ukeGeeks.settings.tabs.labelWidth:0;var tabs=tabInfo.tabs;var height=((this._numStrings-1)*ukeGeeks.settings.tabs.lineSpacing)+(2*ukeGeeks.settings.tabs.dotRadius)+ukeGeeks.settings.tabs.bottomPadding;outElement=(typeof(outElement)=='string')?document.getElementById(outElement):outElement;var ctx=ukeGeeks.canvasTools.addCanvas(outElement,this._getWidth(tabs,labelOffset,false),height);var pos={x:ukeGeeks.settings.tabs.dotRadius+labelOffset,y:1+ukeGeeks.settings.tabs.dotRadius};this._drawStaff(ctx,pos,this._getWidth(tabs,labelOffset,true),ukeGeeks.settings.tabs);this._drawNotes(ctx,pos,tabs,ukeGeeks.settings.tabs);if(tabInfo.hasLabels){this._drawLabels(ctx,pos,ukeGeeks.settings.tabs);}},_readTabs:function(ukeStrings){var hasLabels=ukeStrings[this._numStrings-1][0]==this._lastStringName;if(hasLabels){this._stripStringLabels(ukeStrings);}
var frets=this._getFretNumbers(ukeStrings);var symbols=this._getSymbols(ukeStrings);var minLength=this._getMinLineLength(ukeStrings);var guide=this._getGuideLine(symbols,minLength);return{tabs:this._getPackedLines(frets,symbols,guide,minLength),hasLabels:hasLabels};},_getWidth:function(tabs,labelOffset,isTruncate){if(!isTruncate){return(ukeGeeks.settings.tabs.noteSpacing*tabs[0].length)+labelOffset+ukeGeeks.settings.tabs.dotRadius;}
var len=tabs[0].length;var plusDot=ukeGeeks.settings.tabs.dotRadius;if(tabs[0][len-1]=='|'){len-=1;plusDot=0;}
return ukeGeeks.settings.tabs.noteSpacing*len+labelOffset+plusDot;},_stripStringLabels:function(ukeStrings){for(var i=0;i<this._numStrings;i++){ukeStrings[i]=ukeStrings[i].substr(1);}},_getFretNumbers:function(ukeStrings){var reInts=/([0-9]+)/g;var frets=[];for(var i=0;i<this._numStrings;i++){frets[i]=ukeStrings[i].match(reInts);}
return frets;},_getSymbols:function(ukeStrings){var reDoubles=/([0-9]{2})/g;var reSingle=/([0-9])/g;var symbols=[];for(var i=0;i<this._numStrings;i++){symbols[i]=ukeStrings[i].replace(reDoubles,'-*');symbols[i]=symbols[i].replace(reSingle,'*');}
return symbols;},_getMinLineLength:function(ukeStrings){var minLength=10000;for(var i=0;i<this._numStrings;i++){minLength=(ukeStrings[i].length<minLength)?ukeStrings[i].length:minLength;}
return minLength;},_getGuideLine:function(symbols,minLength){var guide='';for(var i=0;i<minLength;i++){if(symbols[0][i]=='|'){guide+='|';}
...
lastGuide=guide;}
return guide;},_getPackedLines:function(frets,symbols,guide,minLength){var packed=[[],[],[],[]];var chrNote='';for(var j=0;j<this._numStrings;j++){var p=0;var f=0;for(var i=0;i<minLength;i++){if(guide[i]!=' '){if(symbols[j][i]=='*'){chrNote=frets[j][f];f++;}
else{chrNote=((guide[i]=='|'))?'|':'-';}
packed[j][p]=chrNote;p++;}}}
return packed;},_drawStaff:function(ctx,pos,length,settings){var offset=settings.lineWidth/2;var x=pos.x+offset;var y=pos.y+offset;ctx.beginPath();for(var i=0;i<this._numStrings;i++){ctx.moveTo(x,y);ctx.lineTo(x+length,y);y+=settings.lineSpacing;}
ctx.strokeStyle=settings.lineColor;ctx.lineWidth=settings.lineWidth;ctx.stroke();ctx.closePath();},_drawNotes:function(ctx,pos,tabs,settings){var c;var center={x:0,y:pos.y};for(var i in tabs){if(i>3){return;}
center.x=pos.x;for(var j in tabs[i]){c=tabs[i][j];if(c=='|'){var jnum=parseInt(j,10);var heavy=(((jnum+1)<(tabs[i].length-1))&&(tabs[i][jnum+1]=='|'))||((jnum==(tabs[i].length-1))&&(tabs[i][jnum-1]=='|'));this._drawMeasure(ctx,{x:center.x,y:pos.y},settings,heavy);}
...
center.y+=settings.lineSpacing;}},_drawMeasure:function(ctx,pos,settings,heavy){var offset=settings.lineWidth/2;ctx.beginPath();ctx.moveTo(pos.x+offset,pos.y);ctx.lineTo(pos.x+offset,pos.y+(this._numStrings-1)*settings.lineSpacing);ctx.strokeStyle=settings.lineColor;ctx.lineWidth=(heavy?4.5:1)*settings.lineWidth;ctx.stroke();ctx.closePath();},_drawLabels:function(ctx,pos,settings){var labels=ukeGeeks.settings.tuning.slice(0).reverse();for(var i=0;i<this._numStrings;i++){ukeGeeks.canvasTools.drawText(ctx,{x:1,y:(pos.y+(i+0.3)*settings.lineSpacing)},labels[i],settings.labelFont,settings.lineColor,'left');}}};;ukeGeeks.overlapFixer=(function(){var _public={};var checkOverlap=function(a,b){if(a.top!=b.top){return false;}

...
_public.setChords(ukeGeeks.chordImport.runBlock(_instruments[0]).chords);};_public.get=function(chordName){for(var i=0;i<_userChords.length;i++){if(chordName==_userChords[i].name){return _userChords[i];}}
if(_offset<1){return _get(chordName);}
for(var i in _map){if(chordName==_map[i].original){var x=_get(_map[i].transposed);if(x){var chrd=new ukeGeeks.data.expandedChord(chordName);chrd.dots=x.dots;return chrd;}}}
...
else{_public.removeClass(element,className);}};var getRegEx=function(className){return new RegExp('(\\s|^)'+className+'(\\s|$)');};_public.trim=function(str){return str.replace(regEx.trim,'');};_public.pack=function(value){return value.replace(regEx.dbleSpace,' ').replace(regEx.trim,'');};_public.getElementsByClass=function(searchClass,node,tag){if(node===null){node=document;}
if(node.getElementsByClassName){return node.getElementsByClassName(searchClass);}
var classElements=[];if(tag===null){tag='*';}
...
return c;};var _log=function(msg){_errs.push(msg);};var _errs=[];var _echoLog=function(){for(var i in _errs){console.log(i+'. '+_errs[i]);}};_public.runLine=function(line){var c=_lineToParts(line);if(!c){return null;}
return _getExpandedChord(c.define,c.adds);};_public.runBlock=function(text){var nL=text.split('\n');if(nL.length<2){nL=text.split('{');}

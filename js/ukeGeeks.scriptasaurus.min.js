...
return ctx;};}
;ukeGeeks.chordBrush=function(){var publics={};publics.init=function(){};publics.plot=function(chordBox,chord,fretBox,fontSettings,colorSettings){var ctx=ukeGeeks.canvasTools.addCanvas(chordBox,fretBox.width,fretBox.height);if(ctx==null){return;}
...
return{first:(min<300)?min:0,last:(max>0)?max:0};};return publics;};
...
for(var j=0;j<e.length;j++){if(e[j].getAttribute('data-chordName')==c.name){brush.plot(e[j],c,ukeGeeks.settings.inlineFretBox,ukeGeeks.settings.inlineFretBox.fonts);}}}};publics.getErrors=function(){return errors;};publics.getIgnoredChords=function(){return ignoreMatchList;};return publics;};

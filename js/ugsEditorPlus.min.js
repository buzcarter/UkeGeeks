...
;ugsEditorPlus.actions=function(){var a={},c={},b=null,h=null,k=[],d=/^([A-G][#b]?)(m|m6|7|m7|dim|maj7|6|sus2|sus4|aug|9)?$/,l={placement:"above",transpose:"up_0"};a.init=function(b){c={songText:document.getElementById("ukeSongText"),songContainer:document.getElementById("ukeSongContainer"),cpmSource:document.getElementById("chordProSource"),scalableArea:document.getElementById("scalablePrintArea")}};a.doAction=function(b,q){switch(b){case "zoomFonts":var p=parseFloat(q,10);c.scalableArea.style.fontSize=
p+"pt";break;case "zoomDiagrams":var p=parseInt(q,10)/100,m=Math.round(225*p),f=ugsEditorPlus.styles.getSheet("ugsEditorCss"),g=f.Find(".scalablePrintArea .ugs-diagrams-wrap canvas");g.style.width=Math.round(p*ukeGeeks.settings.fretBox.width)+"px";g.style.height=Math.round(p*ukeGeeks.settings.fretBox.height)+"px";g=f.Find(".scalablePrintArea .ugs-diagrams-wrap");g.style.width=m+"px";g=f.Find(".scalablePrintArea .ugs-source-wrap");g.style.marginLeft=25+m+"px";break;case "layout":$("body").toggleClass("diagramsOnTop",
"top"==q).toggleClass("diagramsOnSide","left"==q).toggleClass("ugsHideDiagrams","none"==q);break;case "placement":ukeGeeks.toolsLite.setClass(c.songContainer,"ugsInline","inline"==q);(p="miniDiagrams"==q)||ukeGeeks.toolsLite.removeClass(c.songContainer,"ugsInlineDiagrams");p||"miniDiagrams"==l.placement?(ukeGeeks.settings.inlineDiagrams=p,a.run()):p||"miniDiagrams"==l.placement||ukeGeeks.overlapFixer.Fix(c.songText);l.placement=q;break;case "tuning":p=ukeGeeks.definitions.instrument.sopranoUke;m=
"Standard <strong>GCEA</strong> Soprano Ukulele";"baritone"==q&&(p=ukeGeeks.definitions.instrument.baritoneUke,m="Standard <strong>DGBE</strong> Baritone Ukulele");$("#footTuningInfo").html(m);ukeGeeks.scriptasaurus.setTuningOffset(p);a.run();break;case "colors":ugsEditorPlus.themes.set(q);a.run();break;case "transpose":p="u"==q[0]?1:-1;m=parseInt(q[q.length-1],10);var p=p*m,g=[],m="",e;for(e=0;e<k.length;e++)d.test(k[e])?g.push(k[e]):m+=k[e]+", ";if(!(0<m.length)||confirm("Sorry, but some of your chords can't be transposed:\n"+
m+"\n\nDo you want to continue anyway?")){var r=ukeGeeks.transpose.shiftChords(g,p),m=h;for(e=0;e<g.length;e++)f=RegExp("\\["+g[e]+"\\]","g"),m=m.replace(f,"[ugsxx_"+e+"]");for(e=0;e<r.length;e++)f=RegExp("\\[ugsxx_"+e+"\\]","g"),m=m.replace(f,"["+r[e]+"]");f=/^\s*\{\s*(key|k)\s*:\s*(\S*?)\s*\}\s*$/im;f.test(m)&&(g=m.match(f),e=g[2],""!==e&&d.test(e)&&(m=m.replace(f,g[0].replace(e,ukeGeeks.transpose.shift(e,p)))));c.cpmSource.value=m;a.run()}break;case "paper":p=["letter","a4","screen"];m=$("body");
for(f=0;f<p.length;f++)m.removeClass("pageWidth_"+p[f]);m.addClass("pageWidth_"+q);break;case "showEnclosures":ukeGeeks.settings.opts.retainBrackets=q;a.run();break;case "hideCommonChords":ukeGeeks.settings.opts.ignoreCommonChords=q;a.run();break;case "setCommonChords":p=q.split(/[ ,]+/);m=[];for(f=0;f<p.length;f++)g=ukeGeeks.toolsLite.trim(p[f]),0<g.length&&m.push(g);ukeGeeks.settings.commonChords=m;ukeGeeks.settings.opts.ignoreCommonChords&&a.run();break;case "update":a.run(!0);break;default:console.log("Unrecognized "+
b+" > "+q)}};a.run=function(d){d=0<arguments.length&&d;c.songText.innerHTML="<pre>"+c.cpmSource.value+"</pre>";b=ukeGeeks.scriptasaurus.run();1>b.chords.length&&ugsEditorPlus.autoReformat.run(c);if(b&&(ugsEditorPlus.songUi.update(b),d||null===h)){h=c.cpmSource.value;k=b.chords;var a=""!==b.key?b.key:1>b.chords.length?"":b.chords[0],p=document.getElementById("transposeOptions").getElementsByTagName("li"),m,f=-6;l.transpose="up_0";ugsEditorPlus.submenuUi.resetTransposeLabel();for(var g=0;g<p.length;g++)ukeGeeks.toolsLite.removeClass(p[g],
"checked"),m=1>a.length?"":ukeGeeks.transpose.shift(a,f),p[g].getElementsByTagName("em")[0].innerHTML=m,0===f&&ukeGeeks.toolsLite.addClass(p[g],"checked"),f++}};return a}();
...
ugsEditorPlus.reformat=function(){var a={},c=!1,b=function(){this.source="";this.spaceCount=this.wordCount=0;this.words=[];this.lineType=this.chordCount=0},h=/\b(\S+)\b/gi,k=/(\s+)/g,d=/(^\s+)/,l=/\b[A-G][#b]?(m|m6|m7|m9|dim|dim7|maj7|sus2|sus4|aug|6|7|9|add9|7sus4)?\b/,n=/\b(\S+\s*)/g,q=/^\s*(\|{0,2}[A-Gb]?\|{0,2}[-x0-9|:]{4,})/;a.run=function(a){c=!1;var m=[];a=a.replace("\t","    ");a=a.split("\n");for(var f=0;f<a.length;f++){var g=a[f].match(h),e=new b;e.source=a[f];if(null!=g&&0<g.length){e.wordCount=
g.length;e.words=g;for(var r=e,s=0,t=0;t<g.length;t++)g[t].match(l)&&s++;r.chordCount=s}r=a[f].match(k);null!=r&&0<r.length&&(e.spaceCount=r.length);r=e;g=e;1>g.spaceCount+g.wordCount?g=0:null!=g.source.match(q)?g=3:(s=2,0<g.chordCount&&g.wordCount==g.chordCount&&(s=1,c=!0),g=s);r.lineType=g;m.push(e)}f="";for(a=0;a<m.length;)if(e=m[a],r=m[a+1],a++,r&&0!=e.lineType){if(g=3==e.lineType)a:if(a+3>=m.length)g=!1;else{for(g=a;g<a+3;g++)if(3!=m[g].lineType){g=!1;break a}g=!0}if(g)f+="{start_of_tab}\n"+
...
e+"] span").text(p(g,f,a));$("label[for="+e+"]").parents("dt").removeClass("active");b(g,f);return!1},k=function(a){a=$(this);var b=a.attr("for");a.closest("dl").children("dt").removeClass("active");a.closest("dt").addClass("active");$("#"+b).show();d(this,!0);null!=c&&$("#"+c.id).hide();c=null!=c&&c.id==b?null:{id:b};return!1};a.reset=function(a){a.find("dt").removeClass("active");a.find(".event-userSelecting").removeClass("event-userSelecting")};var d=function(a,b){$(a).parents(".enablePseudoSelects").toggleClass("event-userSelecting",
b)},l=function(a){if(!$(a.target).is("a"))return $(this).find("dd").hide(),c=null,!1},n=function(a){$(".arrowBox").fadeOut(270);ugsEditorPlus.topMenus.makeAllInactive()},q={zoomDiagrams:["Tiny","Small","Medium","Large","Stupid Large"],layout:["Reference diagrams on left","Reference diagrams at top","No reference diagrams"],placement:["Chord names inline with lyrics","Chord names above lyrics","Names & diagrams above lyrics"],tuning:["Soprano (GCEA) tuning","Baritone (DBEA) tuning"]},p=function(a,
b,c){var e=c.index();switch(a){case "paper":return"Width "+c.text();case "zoomFonts":return"Font size "+b+"pt";case "zoomDiagrams":return q.zoomDiagrams[e]+" diagrams";case "colors":return ugsEditorPlus.themes.getDescription(b);case "transpose":return"up_0"==b?"Original key":c.text().replace(" ",' steps - "')+'"';default:return q[a][e]}};a.resetTransposeLabel=function(){$("label[for=transposePicker] span").text("Original key")};return a}();
ugsEditorPlus.newSong=function(){var a={},c=!1,b="";a.init=function(a){b=a;$("#newSongBtn").click(function(a){a=$("#songTitle");var b=a.val().trim();a.val(b);a=2<b.length;k(!a,"Song's title is required<br/><em>(you may change it later, must be at least 2 characters)</em>");a&&h()});$("#openNewDlgBtn").click(function(a){$("#songTitle, #songArtist").val("");$("#newSongForm").fadeIn();$("#songTitle").focus()});$("#hideNewSongBtn").click(d);$("#newSongForm").bind("keydown",l);$spinner=$("#loadingSpinner");
$spinner.hide();$(document).ajaxStart(function(){$spinner.show();c=!0}).ajaxStop(function(){$spinner.hide();c=!1})};var h=function(){if(!c){var a={handler:"ugs_new",songTitle:$("#songTitle").val(),songArtist:$("#songArtist").val()};$.ajax({url:b,type:"POST",dataType:"json",data:JSON.stringify(a),success:function(a){k(a.HasErrors,a.Message);a.HasErrors||(document.location.href=a.ContinueUri)}})}},k=function(a,b){var c=$("#newSongForm .errorMessage");a?(c.show().html(b),$("#songTitle").focus()):c.hide()},
d=function(a){$("#newSongForm").fadeOut()},l=function(a){27==a.which&&d()};return a}();
...
ugsEditorPlus.typeahead=function(){var a={},c=[],b={},h="",k,d=[],l=/\s{2,}/g,n=/([^a-z0-9]+)/gi,q=/['`\u2019-]/g,p=function(){$("li").each(function(a){var e=$(this);a=m(e.text());var g=e.children("a").attr("href"),f=g.toLowerCase(),e=e.children("a").html(),e=e.replace('<strong class="','<span class="bigger ').replace("</strong>","</span>");b[f]={html:e,searchText:a,code:f,href:g};c.push(f)})},m=function(a){return a.toLowerCase().replace(q,"").replace(n," ").replace(l," ").trim()},f=function(a,b){h=
...
"<strong>$1</strong>"))});return a.html()};a.initialize=function(){p();$("#quickSearch").typeahead({source:f,updater:g,matcher:e,sorter:r,highlighter:s,minLength:2,items:50}).val("").focus()};return a};
ugsEditorPlus.resize=function(){var a={},c=null,b=null,h=null,k=null,d=!1,l=!1,n=function(a){c=$(a);$("body").append('<div id="aceHeader"><button class="aceSideBtn" title="Show options &amp; help"><span></span><span></span><span></span></button><strong>Edit Song</strong><a href="#exit-fullscreen">Exit fullscreen</a></div><div id="aceEditor"></div><div id="aceHelp"></div>');b=$("#aceEditor");b.fadeOut(1);h=$("#aceHelp");$("#aceHeader a").click(function(a){a.preventDefault();g()});$("#aceHeader button").click(q)},
q=function(a){a.preventDefault();p(!l)},p=function(a){(l=a)?(h.animate({left:0},400),b.animate({left:"350px"},400)):(h.animate({left:"-350px"},400),b.animate({left:0},400))},m=function(){d=!0;$("html").addClass("aceEditorActive");$(".overlay").fadeOut(300);null===k?LazyLoad.js("/js/ace/ace.js",function(){k=ace.edit("aceEditor");k.setTheme("ace/theme/idle_fingers");k.getSession().setMode("ace/mode/chordpro");k.setOptions({enableBasicAutocompletion:!0,enableSnippets:!0});k.completers=[ugsAce.chordCompleter];
f();h.html(ugsAce.helpHtml)}):f()},f=function(){b.fadeIn(550);k.setValue($("#chordProSource").val());k.gotoLine(1);h.fadeIn(1)},g=function(){d=!1;c.show();b.fadeOut(550);h.fadeOut(550);null!==k&&$("#chordProSource").val(k.getValue());$("html").removeClass("aceEditorActive");p(!1)};a.toggle=function(a){null===c&&n(a);d?g():m();return!1};return a}();
...
ugsChordBuilder.cursorCanvas=function(){var a=ugsChordBuilder.settings.cursor,c=ugsChordBuilder.settings.dot,b={},h=null,k=null,d=!1,l=!0,n=1,q={x:0,y:0};b.init=function(a){h=a;p()};var p=function(){k=new Image;k.onload=function(){d=!0};k.src=a.imageUri};b.setCursor=function(a,b){l=a;n=b};b.draw=function(b){var f=a.radius+a.strokeWidth;h.clearRect(q.x-f,q.y-f,f+50,f+60);!d||l?(h.beginPath(),h.arc(b.x,b.y,a.radius,0,2*Math.PI,!1),h.fillStyle=a.fillColor,h.fill(),h.lineWidth=a.strokeWidth,h.strokeStyle=
a.strokeColor,h.stroke()):(h.drawImage(k,b.x,b.y),h.font=c.fontWeight+" "+c.fontSize+"px "+c.fontFamily,h.textAlign="left",h.fillStyle="black",h.fillText(n,b.x+0.8*k.width,b.y+k.height));q=b};return b}();
ugsChordBuilder.chordCanvas=function(){var a=ugsChordBuilder.entities,c=ugsChordBuilder.settings.centerAnchor,b=ugsChordBuilder.settings.anchorPos,h=ugsChordBuilder.settings.fretLabel,k=ugsChordBuilder.settings.stringLabel,d=ugsChordBuilder.settings.fretBoard,l={},n=null,q=null;l.init=function(a,b){n=a;q=b;c(q)};l.draw=function(c,m){n.clearRect(0,0,q.width,q.height);for(var f=new a.Position(b.x-0.3*h.fontSize,b.y+h.fontSize),g=1<m?h.color:h.lightColor,e=0;e<d.numFrets;e++){var r=m+e,l=f;n.font=h.fontSize+
...
a[e].fret);return{max:b,min:900>c?c:b}},n=function(a){return 0<a?h+a:0},q=function(a,b){for(var c=0;c<a.length;c++)if(a[c].fret==b)return a[c].finger;return 0};b.getPrimaryFrets=function(a){h=a-1;a=d();for(var b=[],c=0;c<a.length;c++){var e=l(a[c].dots);b.push(n(e.max))}return b};b.getDefinition=function(a,b){var c=(a=p(a))&&0<a.length?a:"CHORDNAME",e="",k="",s="";h=b-1;for(var t=d(),x=0;x<t.length;x++){var w=l(t[x].dots),e=e+(n(w.max)+" "),k=k+(q(t[x].dots,w.max)+" ");w.max!=w.min&&(s+=" add: string "+
t[x].string+" fret "+n(w.min)+" finger "+q(t[x].dots,w.min))}return("{define: "+c+" frets "+e+" fingers "+k+s+"}").replace(/\s+/g," ").replace(" }","}")};b.getDefinitionHtml=function(a,c){a=p(a);var d=b.getDefinition(a,c),d=d.replace(" "+a," X07Myq791wso01"),d=d.replace(/\b(\d+)\b/g,'<span class="chordPro-X07MX001number">$1</span>'),d=d.replace(/\b(frets?|fingers?|string)\b/g,'<span class="chordPro-X07MX001attribute">$1</span>'),d=d.replace(/\b(define|add)\b/g,'<span class="chordPro-X07MX001keyword">$1</span>');
return d.replace("X07Myq791wso01",'<span class="chordPro-string">'+a+"</span>").replace(/X07MX001/g,"").replace(/ +/g," ")};var p=function(a){a=a.replace(/^\s*(.*?)\s*$/,"$1").replace(/\s+/g,"-");/^(frets|fingers|add:)$/i.test(a)&&(a="");return a.substring(0,c.nameMaxLength)};return b}();
ugsChordBuilder.chooserList=function(){var a={},c=[],b=null,h=0,k=null,d=function(){},l=null,n={showText:!1,height:50,width:40,fretSpace:9,stringSpace:7,dotRadius:3,lineWidth:1,topLeftPos:{x:10,y:2},xWidth:0.7*7,xStroke:1.4},q={dot:"8pt Arial",text:"8pt Arial",fret:"8pt Arial"},p={fretLines:"#EED18E",dots:"#551D00",dotText:"#ffffff",text:"#000000",fretText:"#EED18E",xStroke:"#551D00"},m=function(a,b){this.id=h++;this.name=a;this.definition=b};a.init=function(a){d=a;b=document.getElementById("cdBldPick");
...
'" title="Edit this definition" class="ugs-grouped">'+e+"</li>"},s=function(a){var b=e(a);a=z(a);a=ukeGeeks.chordImport.runLine(a.definition);null==l&&(l=new ukeGeeks.chordBrush);l.plot(b,a,n,q,p)},t=function(a,b,c){var e=a.replace(b,c);return e==a?a:t(e,b,c)},x=function(a,b){var c=document.getElementById("chordProSource");c.value=t(c.value,a,b);ugsEditorPlus.actions.run()},w=function(a){for(var b=0;b<c.length;b++)if(""+c[b].id==a)return b;return-1},z=function(a){a=w(a);return 0<=a?c[a]:null},A=function(a,
...
'<option value="'+n+'">'+n+"</option>";h.innerHTML=k;h.addEventListener("change",x,!1);document.getElementById("cdBldChordName").addEventListener("keyup",t,!1);c=document.getElementById("cdBldDotsBtn");b=document.getElementById("cdBldFingersBtn");c.addEventListener("click",r,!1);b.addEventListener("click",r,!1);document.getElementById("cdBldShowOutputBtn").addEventListener("click",p,!1);document.getElementById("cdBldCancelBtn").addEventListener("click",m,!1);document.getElementById("cdBldSaveBtn").addEventListener("click",
f,!1);document.getElementById("toolboxSlideUpBtn").addEventListener("click",w,!1);document.getElementById("toolboxSlideDownBtn").addEventListener("click",w,!1);q();document.getElementById("cdBldEditorSurface").addEventListener("mousemove",z,!1);a.addEventListener("click",A,!1);ugsChordBuilder.chordCanvas.init(d,a);ugsChordBuilder.cursorCanvas.init(e);y();u();ugsChordBuilder.chooserList.init(g);return!0};var q=function(){l++;4<l&&(l=0);document.getElementById("cdBldBtnFingerName").innerHTML=n[l]+" ("+
l+")";document.getElementById("cdBldBtnDiagram").className="fingerToolImage finger"+l},p=function(a){s(document.getElementById("cdBldOutputBox"),"collapseOutput",!a.target.checked)},m=function(a){a.preventDefault();e();ugsChordBuilder.chooserList.show(!0)},f=function(a){a.preventDefault();a={name:k,startingFret:h,dots:ugsChordBuilder.fretDots.getDots(),definition:ugsChordBuilder["export"].getDefinition(k,h)};ugsChordBuilder.chooserList.save(a)||(a=document.getElementById("cdBldChordName"),a.focus(),
...
LazyLoad=function(a){function c(b,c){var d=a.createElement(b),f;for(f in c)c.hasOwnProperty(f)&&d.setAttribute(f,c[f]);return d}function b(a){var b=p[a],c,d;b&&(c=b.callback,d=b.urls,d.shift(),m=0,d.length||(c&&c.call(b.context,b.obj),p[a]=null,f[a].length&&k(a)))}function h(){var b=navigator.userAgent;n={async:!0===a.createElement("script").async};(n.webkit=/AppleWebKit\//.test(b))||(n.ie=/MSIE|Trident/.test(b))||(n.opera=/Opera/.test(b))||(n.gecko=/Gecko\//.test(b))||(n.unknown=!0)}function k(e,
g,k,m,x){var w=function(){b(e)},z="css"===e,A=[],v,y,u,B;n||h();if(g)if(g="string"===typeof g?[g]:g.concat(),z||n.async||n.gecko||n.opera)f[e].push({urls:g,callback:k,obj:m,context:x});else for(v=0,y=g.length;v<y;++v)f[e].push({urls:[g[v]],callback:v===y-1?k:null,obj:m,context:x});if(!p[e]&&(B=p[e]=f[e].shift())){q||(q=a.head||a.getElementsByTagName("head")[0]);g=B.urls.concat();v=0;for(y=g.length;v<y;++v)k=g[v],z?u=n.gecko?c("style"):c("link",{href:k,rel:"stylesheet"}):(u=c("script",{src:k}),u.async=
!1),u.className="lazyload",u.setAttribute("charset","utf-8"),n.ie&&!z&&"onreadystatechange"in u&&!("draggable"in u)?u.onreadystatechange=function(){/loaded|complete/.test(u.readyState)&&(u.onreadystatechange=null,w())}:z&&(n.gecko||n.webkit)?n.webkit?(B.urls[v]=u.href,l()):(u.innerHTML='@import "'+k+'";',d(u)):u.onload=u.onerror=w,A.push(u);v=0;for(y=A.length;v<y;++v)q.appendChild(A[v])}}function d(a){var c;try{c=!!a.sheet.cssRules}catch(f){m+=1;200>m?setTimeout(function(){d(a)},50):c&&b("css");return}b("css")}
function l(){var a=p.css,c;if(a){for(c=g.length;0<=--c;)if(g[c].href===a.urls[0]){b("css");break}m+=1;a&&(200>m?setTimeout(l,50):b("css"))}}var n,q,p={},m=0,f={css:[],js:[]},g=a.styleSheets;return{css:function(a,b,c,d){k("css",a,b,c,d)},js:function(a,b,c,d){k("js",a,b,c,d)}}}(this.document);
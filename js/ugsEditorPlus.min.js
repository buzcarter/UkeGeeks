...
return null;};};
;ugsEditorPlus.optionsDlg=new function(){var run=null;var _ele={};this.init=function(actionsController,elements){run=actionsController.run;_ele={docBody:elements.docBody,inputIgnoreList:document.getElementById('commonChordList'),chkIgnore:document.getElementById('chkIgnoreCommon'),pageWidth:document.getElementById('pageWidth'),chkEnclosures:document.getElementById('chkEnclosures')};restoreDefaults();_ele.pageWidth.onchange=function(){doSetWidth(this.value);};_ele.chkEnclosures.onclick=function(){doSetEnclosure(!this.checked);};_ele.inputIgnoreList.onchange=function(){doSetCommon();};_ele.chkIgnore.onclick=function(){doIgnoreChkClk(this.checked);};};var restoreDefaults=function(){_ele.inputIgnoreList.value=ukeGeeks.settings.commonChords.join(", ");_ele.pageWidth.value='letter';_ele.chkIgnore.checked=ukeGeeks.settings.opts.ignoreCommonChords;_ele.chkEnclosures.checked=!ukeGeeks.settings.opts.retainBrackets;};var doSetWidth=function(value){var opts=['letter','a4','screen'];for(var i=0;i<opts.length;i++){ukeGeeks.toolsLite.removeClass(_ele.docBody,'pageWidth_'+opts[i]);}
ukeGeeks.toolsLite.addClass(_ele.docBody,'pageWidth_'+value);};var doSetEnclosure=function(isVisible){ukeGeeks.settings.opts.retainBrackets=isVisible;run();};var doIgnoreChkClk=function(isIgnore){ukeGeeks.settings.opts.ignoreCommonChords=isIgnore;run();};var doSetCommon=function(){var inputList=_ele.inputIgnoreList.value.split(/[ ,]+/);var chordList=[];for(var i=0;i<inputList.length;i++){var c=ukeGeeks.toolsLite.trim(inputList[i]);if(c.length>0){chordList.push(c);}};ukeGeeks.settings.commonChords=chordList;if(_ele.chkIgnore.checked){run();}};};
...
var lineObj=function(){this.source='';this.wordCount=0;this.spaceCount=0;this.words=[];this.chordCount=0;this.lineType=enums.lineTypes.blank;};var re={words:/\b(\S+)\b/gi,spaces:/(\s+)/g,leadingSpace:/(^\s+)/,chordNames:/\b[A-G][#b]?(m|m6|m7|m9|dim|dim7|maj7|sus2|sus4|aug|6|7|9|add9|7sus4)?\b/,chrdBlock:/\b(\S+\s*)/g,tabs:/^\s*(\|{0,2}[A-Gb]?\|{0,2}[-x0-9|:]{4,})/};this.run=function(text){_hasChords=false;var lines=read(text);return merge(lines);};this.hasChords=function(){return _hasChords;};var read=function(text){var lineAry=[];text=text.replace(' ','    ');var lines=text.split('\n');for(var i=0;i<lines.length;i++){var words=lines[i].match(re.words);var l=new lineObj;l.source=lines[i];if((words!=null)&&(words.length>0)){l.wordCount=words.length;l.words=words;l.chordCount=countChords(words);}
...
steps++;}};};
;ugsEditorPlus.newSong=(function(){var _ajaxUri='';var publics={};var _isUpdating=false;publics.init=function(ajaxUri){_ajaxUri=ajaxUri;$('#newSongBtn').click(function(e){if(doValidate(this)){doPost();}});$('#openNewDlgBtn').click(function(e){$('#newSongForm').fadeIn();$('#songTitle').focus();});$('#hideNewSongBtn').click(function(e){$('#newSongForm').fadeOut();});$spinner=$("#loadingSpinner");$spinner.hide();$(document).ajaxStart(function(){$spinner.show();_isUpdating=true;}).ajaxStop(function(){$spinner.hide();_isUpdating=false;});};var doAjaxOk=function(data){showErrors(data.HasErrors,data.Message);if(data.HasErrors){return;}
document.location.href=data.ContinueUri;};var doPost=function(){if(_isUpdating){return;}
var data={'handler':'ugs_new','songTitle':$('#songTitle').val(),'songArtist':$('#songArtist').val()};$.ajax({url:_ajaxUri,type:"POST",dataType:'json',data:JSON.stringify(data),success:function(data){doAjaxOk(data);}});};var doValidate=function(){var $title=$('#songTitle');var title=$title.val().trim();$title.val(title);var ok=title.length>2;showErrors(!ok,'Song\'s title is required<br/><em>(you may change it later, must be at least 2 characters)</em>');return ok;};var showErrors=function(hasErrors,message){var $err=$('#newSongForm .errorMessage');if(hasErrors){$err.show().html(message);$('#songTitle').focus();}else{$err.hide();}};return publics;})();
;ugsEditorPlus.updateSong=(function(){var _ajaxUri='';var _filename='';var publics={};var _isUpdating=false;publics.init=function(ajaxUri,filename){_ajaxUri=ajaxUri;_filename=filename;$('#saveBtn').click(function(event){doUpdate();return false;});$spinner=$("#loadingSpinner");$spinner.hide();$(document).ajaxStart(function(){$spinner.show();_isUpdating=true;}).ajaxStop(function(){$spinner.hide();_isUpdating=false;});};var doUpdate=function(){if(_isUpdating){return;}
var data={'handler':'ugs_update_81jr','filename':_filename,'song':$('#chordProSource').val()};$.ajax({url:_ajaxUri,type:"POST",dataType:'json',contentType:"application/json; charset=utf-8",data:JSON.stringify(data),success:function(data){doAjaxOk(data);}});};var doAjaxOk=function(data){$('#sourceFeedback').html(data.Message);};return publics;})();
;ugsEditorPlus.typeahead=function(){var _keysList=[];var _keysToDetailsDict={};var _scrubbedQuery='';var _regex;var _words=[];var listFromHtml=function(){$('li').each(function(index){var $this=$(this);var plainText=$this.text().replace(/\s\s+/g,' ').trim();plainText=plainText.toLowerCase();var href=$this.children('a').attr('href');var key=href.toLowerCase();var html=$this.children('a').html();html=html.replace('<strong class="','<span class="bigger ').replace('</strong>','</span>');_keysToDetailsDict[key]={html:html,searchText:plainText,code:key,href:href};_keysList.push(key);});};var _ta_source=function(query,process){_scrubbedQuery=query.trim().toLowerCase();_words=_scrubbedQuery.split(' ');var regGroup='';for(var i=0;i<_words.length;i++){_words[i]=_words[i].trim();if(_words[i].length>0){regGroup+=(regGroup.length>0?'|':'')+_words[i];}};_regex=new RegExp('('+regGroup+')','gi');process(_keysList);};var _ta_updater=function(item){window.location=_keysToDetailsDict[item].href;return _keysToDetailsDict[item].searchText;};var _ta_matcher=function(item){var detailed=_keysToDetailsDict[item];for(var i=0;i<_words.length;i++){if(detailed.searchText.indexOf(_words[i])==-1){return false;}}
return true;};var _ta_sorter=function(items){return items.sort(function(a,b){return(_keysToDetailsDict[a].searchText>_keysToDetailsDict[b].searchText);});};var _ta_highligher=function(item){var $temp=$('<div/>').html(_keysToDetailsDict[item].html);$('em, .bigger',$temp).each(function(index){var $ele=$(this);var text=$ele.html();$ele.html(text.replace(_regex,"<strong>$1</strong>"))});return $temp.html();};return{initialize:function(){listFromHtml();$('#quickSearch').typeahead({source:_ta_source,updater:_ta_updater,matcher:_ta_matcher,sorter:_ta_sorter,highlighter:_ta_highligher,minLength:2,items:50}).val('').focus();},};};
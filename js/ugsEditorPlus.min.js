...
$body.addClass('pageWidth_'+value);};var doTuning=function(value){var tuning=ukeGeeks.definitions.instrument.sopranoUke,msg='Standard <strong>GCEA</strong> Soprano Ukulele';if(value=='baritone'){tuning=ukeGeeks.definitions.instrument.baritoneUke;msg='Standard <strong>DGBE</strong> Baritone Ukulele';}
$('#footTuningInfo').html(msg);ukeGeeks.scriptasaurus.setTuningOffset(tuning);_public.run();};var doTranspose=function(value){var sign=value[0]=='u'?1:-1;var steps=parseInt(value[value.length-1],10);transpose(sign*steps);};var transpose=function(steps){var safeChords=[],bad='',i;for(i=0;i<_originalChords.length;i++){if(_re.safe.test(_originalChords[i])){safeChords.push(_originalChords[i]);}
...
var newChords=ukeGeeks.transpose.shiftChords(safeChords,steps);var s=_sourceOriginal;var r;for(i=0;i<safeChords.length;i++){r=new RegExp('\\['+safeChords[i]+'\\]','g');s=s.replace(r,'[ugsxx_'+i+']');}
for(i=0;i<newChords.length;i++){r=new RegExp('\\[ugsxx_'+i+'\\]','g');s=s.replace(r,'['+newChords[i]+']');}
...
ukeGeeks.settings.commonChords=cleanList;if(ukeGeeks.settings.opts.ignoreCommonChords){_public.run();}};return _public;}());;ugsEditorPlus.songUi=(function(){var _public={};var trySet=function(id,value){var hasValue=value&&(value.length>0);var h=document.getElementById(id);if(!h){return;}
...
if(this.Rules[i].selectorText.toLowerCase()==selector){return this.Rules[i];}}
...
return 0;};this.getPrimaryFrets=function(startingFret){_fretOffset=startingFret-1;var dotsPerString=getStringDots();var primaries=[];for(var i=0;i<dotsPerString.length;i++){var minMax=getMinMax(dotsPerString[i].dots);primaries.push(fretNumber(minMax.max));}
...
html=lines[i]+'\n'+html;}
e.value=html;ugsEditorPlus.actions.run();};var dictionaryGetIndex=function(id){for(var i=0;i<_chordDictionary.length;i++){if(''+_chordDictionary[i].id==id){return i;}}
